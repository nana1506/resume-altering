...
import { Sparkles, Mail, Lock, AlertCircle, CheckCircle2, Loader2, ArrowRight } from 'lucide-react';
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        // Confirmation email required or user created
        setSuccessMsg('Registration successful! If confirmation is enabled, please verify your email before logging in.');
        setTimeout(() => {
          router.push('/login');
        }, 3000);
      }
    ...
    <div className="min-h-[75vh] flex items-center justify-center py-8">
      <div className="w-full max-w-md bg-white border border-slate-200 rounded-3xl p-8 shadow-xl shadow-slate-200/50 space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-slate-500">Start optimizing your CV for any job vacancy</p>
        </div>
