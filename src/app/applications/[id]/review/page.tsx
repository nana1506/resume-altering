...
export default function ReviewChecklistPage() {
  ...
  const [predictedScoreMax, setPredictedScoreMax] = useState<number | null>(null);
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        setPredictedScoreMax(data.predicted_match_score ?? (data.match_score ? Math.min(98, data.match_score + 24) : 94));
  ...
  // Calculate dynamic predicted score based on applied changes
  const baseScore = matchScore ?? 68;
  const maxScore = predictedScoreMax ?? Math.min(98, baseScore + 26);
  
  const currentPredictedScore = useMemo(() => {
    if (totalCount === 0) return baseScore;
    const ratio = checkedCount / totalCount;
    return Math.round(baseScore + (maxScore - baseScore) * ratio);
  }, [baseScore, maxScore, checkedCount, totalCount]);

  const scoreImprovement = currentPredictedScore - baseScore;
  ...
  const isHighMatch = baseScore >= 80;
  const isModerateMatch = baseScore >= 60 && baseScore < 80;
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-16">
      ...
      {/* ========================================================= */}
      {/* SECTION 1: ATS MATCH SCORE & SUMMARY (BEFORE ALTERING)    */}
      {/* ========================================================= */}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            ...
            <div className={`w-24 h-24 rounded-2xl flex flex-col items-center justify-center shrink-0 shadow-md ${
              ...
              <span className="text-3xl font-extrabold tracking-tight">{baseScore}%</span>
              <span className="text-[10px] uppercase font-bold tracking-wider opacity-90">Current Fit</span>
            </div>
      ...
      {/* ========================================================= */}
      {/* SECTION 4: PREDICTED MATCH SCORE IMPROVEMENT (BEFORE & AFTER) */}
      {/* ========================================================= */}
      <div className="glass-card rounded-3xl p-6 sm:p-8 space-y-6 border-2 border-emerald-500/30 bg-gradient-to-b from-white to-emerald-50/30 shadow-lg">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div className="space-y-1">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-100 text-emerald-800 text-xs font-bold">
              <TrendingUp className="w-4 h-4" />
              <span>Projected ATS Score Boost</span>
            </div>
            <h3 className="text-xl font-extrabold text-slate-900">
              Score Improvement: Before vs After Altering
            </h3>
            <p className="text-xs sm:text-sm text-slate-500">
              Real-time projection based on your {checkedCount} selected alteration{checkedCount === 1 ? '' : 's'}.
            </p>
          </div>

          {scoreImprovement > 0 && (
            <div className="inline-flex items-center gap-1.5 px-4 py-2 rounded-2xl bg-emerald-600 text-white font-extrabold text-sm shadow-md shadow-emerald-500/25">
              <Zap className="w-4 h-4 fill-white" />
              <span>+{scoreImprovement}% Match Increase</span>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Before Altering Card */}
          <div className="p-5 rounded-2xl bg-white border border-slate-200 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-400">
                1. Original CV (Before)
              </span>
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-extrabold text-slate-700">{baseScore}%</span>
              <span className="text-xs text-slate-500 font-medium">Initial ATS Match</span>
            </div>
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              Contains missing keyword gaps and unaligned terminology from original uploaded resume.
            </p>
          </div>

          {/* After Altering Card (Predicted) */}
          <div className="p-5 rounded-2xl bg-gradient-to-tr from-emerald-50 to-teal-50 border-2 border-emerald-300 space-y-3 relative overflow-hidden shadow-sm">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-emerald-800 flex items-center gap-1">
                <Sparkles className="w-3.5 h-3.5 text-emerald-600" />
                <span>2. Tailored CV (After)</span>
              </span>
              <span className="text-xs font-bold px-2 py-0.5 rounded-md bg-emerald-600 text-white">
                {currentPredictedScore >= 85 ? 'Top Candidate' : 'Target Match'}
              </span>
            </div>

            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-extrabold text-emerald-700">{currentPredictedScore}%</span>
              <span className="text-xs text-emerald-800 font-semibold flex items-center gap-1">
                <span>Predicted Match</span>
                {scoreImprovement > 0 && (
                  <span className="text-emerald-600 font-bold">(+{scoreImprovement}%)</span>
                )}
              </span>
            </div>
            ...
            <div className="w-full bg-emerald-200/60 h-2.5 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${currentPredictedScore}%` }}
              />
            </div>

            <p className="text-xs text-emerald-900 leading-relaxed">
              {checkedCount > 0
                ? `Incorporates ${checkedCount} tailored bullet enhancements bridging ${notExistsCount + diffTermsCount} critical skills and tools.`
                : 'Check suggestions above to see the predicted score increase.'}
            </p>
          </div>
        </div>
      </div>
      ...
      {/* ========================================================= */}
      {/* SECTION 5: GENERATE CV ACTION CARD                        */}
      {/* ========================================================= */}
      <div className="sticky bottom-4 z-20 glass-panel p-4 sm:p-5 rounded-2xl border border-indigo-200 shadow-xl flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h4 className="font-bold text-slate-900 text-sm sm:text-base">Ready to download your tailored CV?</h4>
            {scoreImprovement > 0 && (
              <span className="text-xs font-bold text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-full">
                {baseScore}% → {currentPredictedScore}% ATS Fit
              </span>
            )}
          </div>
          <p className="text-xs text-slate-500">
        ...
        >
          {isGenerating ? (
            ...
            <>
              ...
              <span>Generate CV ({checkedCount} changes • {currentPredictedScore}%)</span>
